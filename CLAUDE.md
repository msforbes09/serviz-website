...
A variable that is defined but blank counts as unset, so its default applies — a dashboard field left empty arrives as `""`, and a schema default only fills in for `undefined`. The site URL accepts a bare hostname and assumes https, and falls back to Vercel's injected production domain when it is not set explicitly. That fallback reads a server-side variable, which is only safe because `env` is imported by the root layout, sitemap and robots and never by a client component. Importing it into a client component would make the browser and the server disagree.
